...
|-------------|----------|------|------|----------|------|
| HTTP Server | `cmd/server/` | 路由、请求解析、响应、graceful shutdown | `main.go` → `server.New()` → `server.Run()` | config, role, session | medium |
...
| Web 前端 | `web/` | 设定页、对话页、报告页 | Phase 1 待实现 | HTTP API | low |
...
|------|----------------|----------|----------|------|
| 全量构建 | `go build ./...` | 所有包 | L1 构建验证 | |
| 全量测试 | `go test ./...` | 所有包 | L2 回归 | |
